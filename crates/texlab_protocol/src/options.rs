...
impl LatexOptions {
    pub fn resolve_output_file(&self, tex_file: &Path, extension: &str) -> Option<PathBuf> {
        let stem = tex_file.file_stem()?.to_str()?;
        ...
        Some(tex_file.parent()?.join(output_directory).join(name))
    }
}
