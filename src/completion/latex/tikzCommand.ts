...
const ITEMS: CompletionItem[] = COMMANDS.map(x =>
  factory.createCommand(x, 'tikz.sty'),
);

type Factory = (database: LatexComponentSource) => CompletionProvider;

export const LatexTikzCommandCompletionProvider: Factory = database =>
  LatexCommandCompletionProvider({
    execute: async ({ relatedDocuments }) => {
      return database
        .relatedComponents(relatedDocuments)
        .some(x => x.fileNames.includes('tikz.sty'))
        ? ITEMS
        : [];
    },
  });
