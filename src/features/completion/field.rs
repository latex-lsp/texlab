...
pub fn complete_fields<'a>(
    ...
    let range = if token.kind() == biblatex::WORD {
        token.text_range()
    } else {
    ...
    if let Some(entry) = biblatex::Entry::cast(parent.clone()) {
        if entry.key()?.name()?.text_range() == token.text_range() {
            return None;
        }
    } else {
        biblatex::Field::cast(parent)?;
    }
