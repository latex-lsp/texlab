...
export function getTypeDocumentation(type: string): string {
  switch (type) {
    case 'article':
      return `An article in a journal, magazine, newspaper, or other periodical which forms a
    ...
    case 'book':
      return `A single-volume book with one or more authors where the authors share credit for
    ...
    case 'mvbook':
      return `A multi-volume \`@book\`. For backwards compatibility, multi-volume books are also
    ...
    case 'inbook':
      return `A part of a book which forms a self-contained unit with its own title. Note that the
    ...
    case 'booklet':
      return `A book-like work without a formal publisher or sponsoring institution. Use the field
    ...
    case 'collection':
      return `A single-volume collection with multiple, self-contained contributions by distinct
    ...
    case 'mvcollection':
      return `A multi-volume \`@collection\`. For backwards compatibility, multi-volume collections
    ...
    case 'incollection':
      return `A contribution to a collection which forms a self-contained unit with a distinct author
    ...
    case 'manual':
      return `Technical or other documentation, not necessarily in printed form. The author or
            editor is omissible.

            Required fields: \`author/editor\`, \`title\`, \`year/date\``;

    case 'misc':
      return `A fallback type for entries which do not fit into any other category. Use the field
            howpublished to supply publishing information in free format, if applicable. The
    ...
    case 'online':
      return `An online resource. \`author\`, \`editor\`, and \`year\` are omissible.
    ...
    case 'patent':
      return `A patent or patent request. The number or record token is given in the number
    ...
    case 'periodical':
      return `An complete issue of a periodical, such as a special issue of a journal. The title of
    ...
    case 'proceedings':
      return `A single-volume conference proceedings. This type is very similar to \`@collection\`.
    ...
    case 'mvproceedings':
      return `A multi-volume \`@proceedings\` entry. For backwards compatibility, multi-volume
    ...
    case 'inproceedings':
      return `An article in a conference proceedings. This type is similar to \`@incollection\`. It
    ...
    case 'report':
      return `A technical report, research report, or white paper published by a university or some
    ...
    case 'thesis':
      return `A thesis written for an educational institution to satisfy the requirements for a degree.
            Use the \`type\` field to specify the type of thesis.

            Required fields: \`author\`, \`title\`, \`type\`, \`institution\`, \`year/date\``;

    case 'unpublished':
      return `A work with an author and a title which has not been formally published, such as
